...
- **Die kWh-Summe sagt jetzt, worauf sie sich stützt.** Tragen nicht alle Fahrten des Zeitraums einen Energiewert (reine GPS-Fahrten haben keinen), steht ein `*` am Wert und der Tooltip nennt das Verhältnis: „Summe aus 8 von 12 Fahrten — der tatsächliche Verbrauch liegt also höher." Ohne diesen Hinweis liest man die Zahl als Gesamtverbrauch, obwohl Fahrten fehlen. Dafür liefert `/api/trips/stats` neu das Feld `energy_trips`.
...
- **Windows als Host (Docker Desktop + WSL2).** Anlass war eine Interessenten-Anfrage. Die Anwendung steckt ohnehin vollständig in Linux-Containern — Windows ist nur der Host, der Stack ist derselbe. Was fehlte, war der Weg dorthin und die Kenntnis der Fallstricke. Beides gibt es jetzt: `deploy/setup-windows.ps1` erzeugt `backend\.env` (inkl. zufälligem `JWT_SECRET` aus dem Krypto-RNG), schreibt eine passende `docker-compose.override.yml` und startet den Stack. `setup.sh` bleibt Linux-only (bash, apt, systemd, certbot).

  **Zwei Einschränkungen, beide an einem echten Docker-Daemon gemessen statt vermutet:**

  1. **Keine Fahrzeugbefehle.** `tesla-http-proxy` braucht den Bind-Mount `/etc/tesla-proxy` und die feste UID 988. Unter Windows bleibt der Service aus; das Backend startet trotzdem, weil sein `depends_on` jetzt auf `required: false` steht (Compose ≥ 2.20). Verifiziert: Mit Windows-Override enthält der Stack nur noch backend/frontend/nginx — **ohne** den sonst fälligen „depends on undefined service"-Abbruch. Alles Lesende bleibt vollständig.
  2. **`host.docker.internal` zeigt unter Docker Desktop woanders hin.** Der Daemon trägt den Namen selbst in die `/etc/hosts` jedes Containers ein, und dieser Eintrag **gewinnt gegen den Netzwerk-Alias** aus der Compose-Datei. Gemessen: mit Eintrag löst der Name auf das Host-Gateway (172.17.0.1) auf, ohne Eintrag auf den Proxy-Container. Signed Commands wären damit still am falschen Ziel gelandet. `PROXY_BASE` in `teslaApi.js` war hart verdrahtet und ist jetzt über `TESLA_PROXY_BASE` überschreibbar — der Linux-Default bleibt unverändert.

  **Weitere Härtung, die auch Linux zugutekommt:** `TESLA_PROXY_CONFIG_DIR` und `TESLA_PROXY_UID`/`GID` ersetzen die hart kodierten Werte `/etc/tesla-proxy` und `988:988`. Neu ist dafür ein `.env.example` in der **Projektwurzel** — mit dem ausdrücklichen Hinweis, dass Compose seine `${...}`-Platzhalter aus *dieser* Datei ersetzt und ein Eintrag in `backend/.env` dafür **wirkungslos** ist (nachgemessen, weil die umgekehrte Annahme näherliegt).

  **Bewusst nicht gebaut:** eine Windows-eigene HTTPS-Lösung. Tesla verlangt eine öffentlich erreichbare HTTPS-Adresse für Anmeldung, Partner-Registrierung und Fleet Telemetry — das gilt für jede Heiminstallation gleichermaßen und steht seit jeher in `14-network-access.*`.

  **Ehrlich ausgewiesen:** Der Docker-Build-Gate baut Linux-Images für amd64/arm64; **ein Windows-Host wird nirgends automatisiert geprüft.** Die Plattform-Tabelle in `docs/02-deployment.*` (×7) führt Windows deshalb mit einer Fußnote „läuft, aber nicht CI-getestet", und die README-Plattformzeile (×7) nennt die Einschränkung „ohne Fahrzeugbefehle" direkt mit.
...
- **„Ich muss oft neu laden, um eine andere Seite zu öffnen" — 429 durch zu knappen Burst am Reverse-Proxy.** Gemessen an der Referenzinstallation: 40 parallele Anfragen auf `/api/health` ergaben 20× 200 und 20× 429; der Header `x-retry-in` weist die Drosselung dem vorgelagerten Traefik zu, nicht der App (deren eigenes Limit meldete zeitgleich noch 93 von 120 frei). Der Traefik-Router für `PathPrefix(/api/)` hing an einer Rate-Limit-Middleware mit `burst: 20` — und dieser Pfad umfasst auch `/api/tiles/{z}/{x}/{y}`, den OSM-Kachel-Proxy. **Damit wiederholte sich auf der Traefik-Ebene exakt der Fehler, der für nginx längst behoben war** (eigene `zone=tiles`, PR #213): Ein Karten-Zoom lädt 50–150 Kacheln in einem Rutsch und leert den Burst-Eimer vollständig. Auch ohne Karte reicht es aber schon: Ein Seitenwechsel im SPA feuert 15–26 API-Calls (Dashboard ~15 inkl. der immer eingehängten Widgets, Laden/Batterie je +11, Routenplaner +18) — mehr, als `burst: 20` durchlässt.
  ...
  **Was der Retry NICHT abdeckt:** Die Karten-Kacheln selbst. Leaflet lädt sie über `<img>`-Elemente (`lib/tiles.js`), nicht über axios — der Interceptor sieht sie nie. Für sie zählt weiterhin allein die Ausnahme am Reverse-Proxy plus der dort schon vorhandene Tile-Retry. Der Frontend-Fix schützt die *übrigen* API-Calls, die bisher als Kollateralschaden mit 429 abgewiesen wurden.
...
- **Telemetry-Fahrten bekommen jetzt SoC, Odometer und Verbrauch.** Sven-Report: 20-km-Fahrt zeigt „Verbrauch –" und „SoC –% → –%". Ursache in `fleetTelemetry.js`: Beim Fahrt-Start/-Ende wurden `start_/end_soc` und `start_/end_odometer_km` nur aus genau der Telemetry-Nachricht gelesen, die den Gear-Wechsel meldet — Tesla streamt SoC/Odometer aber in separaten Nachrichten, die Werte blieben daher fast immer NULL, obwohl sie während der Fahrt laufend als `telemetry_points` gespeichert wurden (Beleg-Fahrt: 578 Punkte, davon 125 mit SoC und 49 mit Odometer). Zusätzlich berechnete der Telemetry-Pfad `energy_used_kwh` überhaupt nie — das tat bisher nur der Polling-Pfad (`dataSync.finishOdometerTrip`), und der stand wegen des erreichten Monats-Caps (1000 Calls) still. Fix: Neuer Nachzug `enrichClosedTrip()` beim Fahrt-Ende füllt SoC/Odometer per COALESCE aus dem ersten/letzten non-NULL-Trackpunkt (analog zum bestehenden Lat/Lon-Nachzug) und berechnet den Verbrauch aus dem SoC-Delta (gleiche Formel wie der Polling-Pfad). Bewusst kein Ledger-Eintrag — System-Schreibpfade ledgern nicht, der Ledger sichert User-Edits ab.

### Hinzugefügt

- **Backfill für bestehende Telemetry-Fahrten.** `backfillTelemetryTrips()` zieht SoC/Odometer/Verbrauch für bereits abgeschlossene Telemetry-Fahrten aus ihren vorhandenen `telemetry_points` nach — bereits gesetzte Werte werden nie überschrieben. Läuft nächtlich als Wartungs-Task (`telemetry_backfill` im Maintenance-Log) und on-demand via `POST /api/system/telemetry-backfill` (Admin-only, auditiert, Limit ≤ 1000 pro Aufruf).
...
- **Ø-Verlust ist gegen Ausreißer geschützt.** Ein Auto, das im Tunnel oder in der Tiefgarage kurz die Verbindung verliert, erzeugt Mini-Schlafphasen — auf %/h hochgerechnet ergäben 2 % in 5 Minuten sonst 24 %/h. In die Kennzahl gehen jetzt nur Phasen ab 60 Minuten ein; Phasen mit negativem Verlust (es wurde geladen) bleiben ebenfalls draußen. In der Ereignisliste sind alle Phasen weiterhin sichtbar.
...
- **CSP für Google Fonts an der richtigen Stelle korrigiert.** Der ausgelieferte `Content-Security-Policy`-Header stammt vom **Host-Edge-nginx** (`deploy/nginx-host.conf.template`), nicht von helmet — der helmet-Fix aus v3.36.6 blieb daher wirkungslos für den echten Header. Jetzt erlaubt das nginx-Template `https://fonts.googleapis.com` (`style-src`) und `https://fonts.gstatic.com` (`font-src`). **Hinweis:** Dieses Host-Template wird nur über `setup.sh` angewendet, nicht vom Deploy — auf dem Live-Host muss `/etc/nginx/sites-available/tesla-carview` einmalig nachgezogen + nginx neu geladen werden.
...
- **Telemetrie-Geschwindigkeit war in mph statt km/h.** Tesla streamt `VehicleSpeed` in **mph**; der Decoder speicherte den Wert aber ungerechnet in der `speed_kmh`-Spalte — im Fahrt-Detail-Schieber standen also mph unter dem Label „km/h" (Werte ~1,6× zu niedrig). Jetzt wird `VehicleSpeed` mit `× 1,60934` auf km/h umgerechnet, konsistent mit Odometer, Live-Endpoint und `dataSync`. Gilt für neue Fahrten; bereits gespeicherte Telemetrie-Punkte behalten ihren alten (mph-)Wert.
- **Diagnose-Log entfernt.** Das temporäre `[FleetTelemetry][diag]`-Log aus v3.35.2 ist raus — der Value-Typ-Fix ist bestätigt (SoC & Leistung werden wieder erfasst: 742 SoC-/213 Leistungs-Punkte in 3 h, vorher 0).
...
- **Setup schlug bei der Admin-Erstellung fehl (`Transaction function cannot return a promise`)** ([#170](https://github.com/KnevS/Tesla-Carview/issues/170)): Der `db.transaction()`-Callback in `/api/setup/init` war `async` — das lehnt better-sqlite3 ab. Der Callback ist jetzt synchron (der bcrypt-Hash lief ohnehin schon außerhalb der Transaktion). Frische Installationen können den Setup-Wizard (Schritt 2) wieder abschließen.
...
- **Fleet-Telemetrie-Daten werden jetzt dekodiert.** Firmware 2026.20.3 sendet die Telemetrie als **FlatBuffers-Envelope mit eingebettetem Protobuf**, nicht als reines Protobuf — der bisherige Decoder scheiterte an jedem Paket (`invalid wire type 4`). Der Receiver (`fleetTelemetry.js`) entpackt den Envelope jetzt mit einem schlanken FlatBuffers-Reader (VIN, Timestamp, Payload) und dekodiert das eingebettete Protobuf. Zusätzlich `Field.Soc` von 12 auf **8** korrigiert (Tesla-Enum). Live verifiziert: PackVoltage/PackCurrent kommen mit plausiblen Werten an (≈391 V / −0,5 A), keine Decode-Fehler mehr. Damit ist die Fleet-Telemetry-Kette Ende-zu-Ende funktionsfähig.
