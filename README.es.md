...
| **Ranking de precios de carga** (v3.50.0) | «¿Dónde cargas más barato?»: ordena los puntos de carga por €/kWh en lugar de por gasto total y calcula el sobrecoste frente a tu electricidad doméstica. Las cargas sin tarifa registrada no cuentan como gratuitas; se indica la base de precios |
| **Balance energético** (v3.49.0) | «¿Cuánto consumes realmente?» — Tesla solo muestra el consumo en marcha. El balance pone lado a lado marcha, de batería a rueda (incl. standby) y de red a rueda (incl. pérdidas de carga); típicamente 15–25 % de diferencia. Corrección del estado de carga con capacidad medida; si el balance no cuadra, no se muestra ninguna cifra en lugar de una engañosa |
| **Eficiencia de carga** (v3.48.0) | ¿Cuánta de la energía consumida llega realmente a la batería? El desglose por banda de potencia muestra qué método de carga sale caro: unos pocos por ciento de pérdida en un wallbox, más de una quinta parte en un enchufe doméstico. Medido con contador MID o estimado a partir de datos del vehículo; las cargas sin datos suficientes quedan sin evaluar |
...
| **Planificador de rutas** | Planificador interactivo con paradas de carga conscientes del SoC, incl. estimación del tiempo de carga y de los kWh; SoC de salida (en vivo o manual), SoC objetivo y meta de carga configurables; meteorología (Open-Meteo), tráfico (HERE Maps), radares (OpenStreetMap) a lo largo de la ruta; vista de mapa con tile proxy. **Lista de deseos del planificador (v3.51.4–.8):** tiempo total del viaje + desglose por tramos, entrada manual de pausas (incluida en la hora de llegada + exportación ICS), filtro por operador de estaciones de carga, comentarios de la comunidad OpenChargeMap sobre las estaciones |
