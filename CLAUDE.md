...
- **Version:** v3.51.3
- **Zuletzt geliefert:**
  - **v3.51.3 (Routenplaner: manuelle Pausen-Eingabe — drittes Ergebnis des Routenplaner-Wunschzettels, S10):** Neue Sektion „Pausen" in `RoutePlanner.vue` — Bezeichnung (optional) + Dauer in Minuten, hinzufügen/entfernen. Fließt in `timingResult` (Ankunfts-/Abfahrtszeit) und `downloadICS()` ein: `totalMin = duration_min + totalChargeMins + totalBreakMins`. Rein clientseitiger State (`breaks` ref), bewusst NICHT mit `/saved-routes` persistiert (im Unterschied zu `waypoints`) — Kernwunsch war die Zeitrechnung, keine dauerhafte Pausen-Verwaltung. **Live verifiziert:** Route Stuttgart→Ulm, Abfahrt 09:00 → ohne Pause X Uhr Ankunft, mit 45-min-Pause verschiebt sich „Voraussichtliche Ankunft" exakt um 45 Minuten (10:55), Pausen-Badge „+45 min" erscheint neben der bestehenden Ladezeit-Anzeige. i18n ×7 neue Keys. **S10-Roadmap Routenplaner:** 1) ✅ Gesamtfahrzeit + Teilstrecken, 2) ✅ Ladedauer-Forecast/kWh, 3) ✅ Pausen-Eingabe (dieser Eintrag), 4) Anbieter-Filter offen (Backend hat `operator` pro Charger schon), 5) Ladestation-Bewertungen offen (neue DB-Tabelle nötig, komplexestes Feature).
