...
- **Version:** v3.51.3
- **Zuletzt geliefert:**
  - **v3.51.3 (Routenplaner: Lademengen-Forecast kWh — zweites Ergebnis des Routenplaner-Wunschzettels, S10):** `planChargingStops()` (routing.js) berechnete `kwh2add` je Stopp schon immer (Grundlage der Ladedauer-Schätzung), gab es aber nie zurück — UI zeigte nur die SoC-Differenz, nicht die Energiemenge. Neu: `stops[].kwh_added` + `total_kwh_added` (Routensumme), im Frontend neben Ladedauer/Leistung + als Summenzeile. i18n ×7 (`routes.kwhAdded`/`totalKwhAdded`, eigener Namespace — **Merke:** `charging.kwhAdded` existierte schon seit v3.50.0 mit Parameter `{value}`, mein neuer `routes.kwhAdded` nutzt bewusst `{kwh}` in einem anderen Namespace, keine Kollision, aber beim Einfügen per Skript den richtigen Namespace/Anker prüfen — ein Ankerpunkt, der nur im PARALLELEN Feature-Branch existiert [hier: `routes.legs` aus PR #251], existiert im eigenen Branch (von main gezweigt) nicht, das Einfüge-Skript läuft dann still folgenlos durch ohne Fehler zu werfen.). **S10-Roadmap Routenplaner:** 1) ✅ Gesamtfahrzeit + Teilstrecken, 2) ✅ Ladedauer-Forecast/kWh (dieser Eintrag), 3) Pausen-Eingabe offen, 4) Anbieter-Filter offen (Backend hat `operator` pro Charger schon), 5) Ladestation-Bewertungen offen (neue DB-Tabelle nötig).
