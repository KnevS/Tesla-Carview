...
- **Version:** v3.51.3
- **Zuletzt geliefert:**
  - **v3.51.3 (Routenplaner: Gesamtreisezeit + Teilstrecken — erstes Ergebnis des Routenplaner-Wunschzettels von Sven, S10):** `RoutePlanner.vue` berechnete Fahrzeit (`routeData.duration_min`) und Ladezeit je Stopp (`stop.charge_minutes`) schon vorher, zeigte aber weder die kombinierte Gesamtreisezeit noch eine Teilstrecken-Aufschlüsselung. Neu: „Gesamtreisezeit"-Zeile (nur bei vorhandenem Ladeplan) + „Teilstrecken"-Liste (Start→WP1, WP1→WP2, …, WPn→Ziel) mit Distanz/Dauer je Segment, nur bei ≥ 1 Zwischenstopp sichtbar. **Backend-Fund:** `fetchValhalla()` (Umgehungsoptionen) fasste alle Legs zu einer Geometrie zusammen und verwarf `duration`/`distance` je Segment — Valhalla liefert das selbst (`data.trip.legs[].summary`), OSRM lieferte `legs` schon immer nativ mit. Beide Pfade liefern jetzt `legs` im selben Format. i18n ×7 neue Keys. **Live gegen echtes OSRM verifiziert** (Stuttgart → Geislingen → Ulm: 2 Teilstrecken 59km/53min + 32km/34min = 90km/1h27min, exakt deckungsgleich mit Gesamtstrecke). **S10-Roadmap (Routenplaner, priorisiert von Sven):** 1) ✅ Gesamtfahrzeit + Teilstrecken (dieser Eintrag), 2) Ladedauer-Forecast (Backend liefert `charge_minutes` schon, UI-Ausbau offen), 3) Pausen-Eingabe, 4) Anbieter-Filter für Ladestationen (Backend hat `operator` pro Charger schon, UI-Filter offen), 5) Ladestation-Bewertungen/Community (neue DB-Tabelle nötig, komplexestes Feature, zuletzt).
