...
For live vehicle data (battery, climate, TPMS, telemetry stream) there is **only one official path**: the Tesla **Fleet API** with app approval via [developer.tesla.com](https://developer.tesla.com/). Current waiting time is **weeks to months**.

**💡 Tesla free tier — typical private use costs €0/month:** Tesla grants **$10 of free credit per account per month** — enough to cover telemetry stream for one vehicle + daily commands. Beyond that it's pay-as-you-go (150,000 stream signals = $1, 1,000 commands = $1, 50 wake-ups = $1). TeslaView is fully wired up — the moment your app is approved, every feature is live instantly. The wait is purely on Tesla's side; TeslaView itself is always free.
...
| **Charging price ranking** (v3.50.0) | "Where do you charge cheapest?" — sorts charging locations by €/kWh rather than total spend and computes the premium against your own home electricity. Charges without a recorded tariff deliberately do not count as free; the price basis is reported |
| **Energy balance** (v3.49.0) | "What do you actually consume?" — Tesla only shows driving consumption. The balance puts driving, battery-to-wheel (incl. standby) and grid-to-wheel (incl. charging losses) side by side; typically 15–25 % apart. State-of-charge correction with measured battery capacity; if the balance does not add up, no figure is shown rather than a misleading one |
| **Charging efficiency** (v3.48.0) | How much of the energy drawn actually reaches the battery? A breakdown by power band shows which charging method is expensive — a few percent loss at a wallbox, over a fifth at a household socket. Calibrated via MID meter or estimated from vehicle data; charges without a solid data basis stay deliberately unrated |
| **Charge planner** (v3.42.0) | Cheapest charging slots before departure from the dynamic tariff (aWattar/Tibber): from target charge, battery capacity, charging power and departure time it picks the cheapest hours — with cost, charging time and savings versus charging now. Pure price-curve analysis, no vehicle access |
| **Company-car cost split** (v3.43.0) | Split home charging into business/private and export a reimbursement PDF — flat rate (German § 3 No. 50 EStG, €30/€70) or mileage share (business km share × home-charging cost). Company cars only, German-language |
| **Solar surplus charging** (v3.44.0) | Charge only from solar surplus: reads the surplus from Home Assistant and derives the recommended charging current — "Apply now" sets the current and starts/stops charging (Fleet API). Fully local, no cloud |
| **Company-car tax** (v3.45.0) | 1% rule vs. logbook method for the taxable benefit — with a date-dependent EV rate (0.25%/0.5%, gross-list-price cap by acquisition date). Pure electric vehicles (BEV) only, guidance calculation, company cars only, German-language |
| **SoH certificate** (v3.46.0) | Battery health certificate as PDF for lease return/resale — range at 100%, degradation rate, projection to 80%, optional SoH in %. Statistical estimate, no warranty |
| **Tamper-proof logbook** (v3.47.0) | Every trip change lands in an HMAC-signed linked hash chain — the change history cannot be silently altered afterwards (GoBD). Integrity check + statement in the tax-office PDF |
...
| **App hub** (v3.9.0) | Curated web apps for the Tesla browser: ARD Audiothek, Deutschlandfunk Live, GoingElectric, OpenChargeMap, Telegram Web, Wikipedia, ABRP — free, no forced account, **deliberately no duplicates** of Tesla-native apps. Admin whitelist per tenant. Admins add their own web apps (v3.40.0). |
...
| **Route planner** | Interactive route planner with SoC-aware charging stops incl. charge time and kWh estimate; departure SoC (live or manual), target SoC and charging target configurable; weather (Open-Meteo), traffic (HERE Maps), speed cameras (OpenStreetMap) along the route; map view with tile proxy. **Personal range (v3.29): arrival SoC from your temperature-dependent consumption instead of WLTP, with a confidence band and "could get tight" warning — pure statistics.** **Route planner wish list (v3.51.4–.8):** total trip time + leg-by-leg breakdown, manual break input (flows into arrival time + ICS export), provider filter for charging stations, OpenChargeMap community comments on charging stations |
...
The script auto-detects the architecture (x86_64, ARM64) and installs everything.
...
All technical procedures documented in this repository — in particular **Battery-Health Companion** (phase 1+2), **OwnTracks validation via Bluetooth trigger** with per-vehicle trip lock, **automatic charge limit per geofenced location**, **curated web app hub for the Tesla browser**, **POI search around a charging session via OSM Overpass**, **automatic reverse geocoding with local cache**, **address-first UI strategy**, and **multi-stage anomaly detection with push recommendations** — are publicly published as of the date of the respective Git commit and constitute "prior art" in the sense of patent and trademark law.

This disclosure is intended to prevent later third-party intellectual-property filings on the same procedures.

Git hashes and commit timestamps are cryptographically verifiable and independently timestamped by GitHub.
