...
| **Περίοδος & επιλογή** (v3.54.0) | Φιλτράρισμα της λίστας διαδρομών ανά περίοδο (γρήγορες επιλογές ή ελεύθερα όρια από/έως) με σύνολο από τον διακομιστή: χιλιόμετρα, ενέργεια (kWh) και πλήθος διαδρομών· η λειτουργία επιλογής αθροίζει τις επιλεγμένες διαδρομές |
...
| **Κατάταξη τιμών φόρτισης** (v3.50.0) | «Πού φορτίζεις φθηνότερα;» — ταξινομεί τα σημεία φόρτισης κατά €/kWh αντί για συνολική δαπάνη και υπολογίζει το επιπλέον κόστος έναντι του οικιακού ρεύματος. Οι φορτίσεις χωρίς καταχωρημένο τιμολόγιο σκόπιμα δεν μετρούν ως δωρεάν· η βάση τιμών αναφέρεται |
| **Ενεργειακό ισοζύγιο** (v3.49.0) | «Πόσο καταναλώνεις στην πραγματικότητα;» — Η Tesla δείχνει μόνο την κατανάλωση οδήγησης. Το ισοζύγιο παραθέτει οδήγηση, από μπαταρία σε τροχό (με αναμονή) και από δίκτυο σε τροχό (με απώλειες φόρτισης)· τυπική διαφορά 15–25 %. Διόρθωση στάθμης φόρτισης με μετρημένη χωρητικότητα· αν το ισοζύγιο δεν βγαίνει, δεν εμφανίζεται νούμερο αντί για παραπλανητικό |
| **Απόδοση φόρτισης** (v3.48.0) | Πόση από την ενέργεια που καταναλώνεται φτάνει πραγματικά στην μπαταρία; Η ανάλυση ανά ζώνη ισχύος δείχνει ποια μέθοδος φόρτισης κοστίζει: λίγα τοις εκατό απώλειες σε wallbox, πάνω από το ένα πέμπτο σε οικιακή πρίζα. Μέτρηση με πιστοποιημένο μετρητή MID ή εκτίμηση από δεδομένα οχήματος· φορτίσεις χωρίς επαρκή δεδομένα παραμένουν σκόπιμα χωρίς αξιολόγηση |
...
| **Φόρος εταιρικού οχήματος** (v3.45.0) | Κανόνας 1 % vs. μέθοδος βιβλίου διαδρομών για την παροχή σε είδος — με συντελεστή EV εξαρτώμενο από την ημερομηνία (0,25 %/0,5 %/1 %, όριο τιμής καταλόγου ανά ημερομηνία αγοράς). Ενδεικτικός υπολογισμός, μόνο εταιρικά οχήματα, στα γερμανικά |
| **Πιστοποιητικό SoH** (v3.46.0) | Πιστοποιητικό υγείας μπαταρίας σε PDF για επιστροφή leasing/μεταπώληση — εμβέλεια στο 100 %, ρυθμός υποβάθμισης, πρόβλεψη έως 80 %, προαιρετικά SoH σε %. Στατιστική εκτίμηση, χωρίς εγγύηση |
| **Απαραβίαστο βιβλίο διαδρομών** (v3.47.0) | Κάθε αλλαγή διαδρομής καταγράφεται σε αλυσιδωτή, HMAC-υπογεγραμμένη hash chain — το ιστορικό αλλαγών δεν μπορεί να αλλοιωθεί εκ των υστέρων απαρατήρητα (GoBD). Έλεγχος ακεραιότητας + δήλωση στο φορολογικό PDF |
...
| **Σχεδιαστής διαδρομής** | Διαδραστικός σχεδιαστής διαδρομής με στάσεις φόρτισης ευαίσθητες σε SoC, συμπεριλαμβανομένης εκτίμησης χρόνου φόρτισης και kWh· SoC αναχώρησης (live ή χειροκίνητο), στόχος SoC και στόχος φόρτισης διαμορφώσιμα· καιρός (Open-Meteo), κίνηση (HERE Maps), κάμερες ταχύτητας (OpenStreetMap) κατά μήκος της διαδρομής· προβολή χάρτη με tile proxy. **Λίστα επιθυμιών σχεδιαστή διαδρομής (v3.51.4–.8):** συνολικός χρόνος ταξιδιού + ανάλυση ανά τμήμα, χειροκίνητη εισαγωγή διαλειμμάτων (ενσωματώνεται στην ώρα άφιξης + εξαγωγή ICS), φίλτρο παρόχου για σταθμούς φόρτισης, σχόλια κοινότητας OpenChargeMap για σταθμούς φόρτισης |
