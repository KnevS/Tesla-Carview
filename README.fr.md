...
Fonctionne sur : **serveurs Linux** (x86_64), **Raspberry Pi 4/5** (ARM64), **Windows** (Docker Desktop + WSL2, sans commandes véhicule), développement local.
...
| **Classement des prix de charge** (v3.50.0) | « Où charges-tu le moins cher ? » — classe les lieux de charge par €/kWh plutôt que par dépense totale et calcule le surcoût par rapport à ton électricité domestique. Les charges sans tarif enregistré ne comptent volontairement pas comme gratuites ; la base tarifaire est indiquée |
| **Bilan énergétique** (v3.49.0) | « Que consommes-tu réellement ? » — Tesla n'affiche que la consommation en roulant. Le bilan met côte à côte conduite, batterie à la roue (veille incluse) et réseau à la roue (pertes de charge incluses) ; écart typique de 15–25 %. Correction de l'état de charge avec capacité mesurée ; si le bilan ne tombe pas juste, aucun chiffre n'est affiché plutôt qu'un chiffre trompeur |
| **Rendement de charge** (v3.48.0) | Quelle part de l'énergie soutirée arrive réellement dans la batterie ? La répartition par palier de puissance montre quel mode de charge coûte cher : quelques pour cent de pertes sur une wallbox, plus d'un cinquième sur une prise domestique. Mesuré par compteur MID ou estimé à partir des données du véhicule ; les charges sans données suffisantes restent non évaluées |
...
| **Planificateur d'itinéraire** | Planificateur d'itinéraire interactif avec arrêts de recharge tenant compte du SoC, incl. estimation du temps de recharge et des kWh ; SoC de départ (en direct ou manuel), SoC cible et cible de recharge configurables ; météo (Open-Meteo), trafic (HERE Maps), radars (OpenStreetMap) le long de l'itinéraire ; vue carte avec proxy de tuiles. **Liste de souhaits du planificateur (v3.51.4–.8) :** temps de trajet total + détail par tronçon, saisie manuelle de pauses (intégrée à l'heure d'arrivée + export ICS), filtre par opérateur pour les bornes, commentaires de la communauté OpenChargeMap sur les bornes |
...
Le script détecte automatiquement l'architecture (x86_64, ARM64) et installe tout.
