...
| **Şarj fiyat sıralaması** (v3.50.0) | „En ucuz nerede şarj ediyorsun?" — şarj noktalarını toplam harcamaya değil €/kWh değerine göre sıralar ve ev elektriğine kıyasla ek maliyeti hesaplar. Tarifesi kayıtlı olmayan şarjlar bilinçli olarak ücretsiz sayılmaz; fiyat temeli belirtilir |
| **Enerji bilançosu** (v3.49.0) | „Gerçekte ne kadar tüketiyorsun?" — Tesla yalnızca sürüş tüketimini gösterir. Bilanço sürüşü, aküden tekerleğe (bekleme dahil) ve şebekeden tekerleğe (şarj kayıpları dahil) yan yana koyar; tipik fark %15–25. Ölçülen akü kapasitesiyle şarj durumu düzeltmesi; bilanço tutmazsa yanıltıcı bir sayı yerine hiçbir şey gösterilmez |
| **Şarj verimliliği** (v3.48.0) | Çekilen enerjinin ne kadarı gerçekten aküye ulaşıyor? Güç bandına göre döküm, hangi şarj yönteminin pahalı olduğunu gösterir: duvar ünitesinde birkaç yüzde kayıp, ev prizinde beşte birden fazla. MID sayacıyla ölçülür veya araç verilerinden tahmin edilir; yeterli veriye sahip olmayan şarjlar bilinçli olarak değerlendirilmez |
| **Şarj planlayıcı** (v3.42.0) | Dinamik tariften (aWattar/Tibber) kalkıştan önceki en ucuz şarj aralıkları: hedef şarj, batarya kapasitesi, şarj gücü ve kalkış saatinden en ucuz saatleri seçer — maliyet, şarj süresi ve hemen şarj etmeye kıyasla tasarrufla. Yalnızca fiyat eğrisi analizi, araca erişim yok |
...
| **Değiştirilemez seyir defteri** (v3.47.0) | Her sürüş değişikliği HMAC ile imzalı, zincirli bir hash zincirine yazılır — değişiklik geçmişi sonradan fark edilmeden değiştirilemez (GoBD). Bütünlük kontrolü + vergi PDF'inde beyan |
...
| **Uygulama merkezi (App hub)** (v3.9.0) | Tesla tarayıcısı için özenle seçilmiş web uygulamaları: ARD Audiothek, Deutschlandfunk Live, GoingElectric, OpenChargeMap, Telegram Web, Wikipedia, ABRP — ücretsiz, hesap zorunluluğu yok, **Tesla'nın yerleşik uygulamalarıyla bilinçli olarak çakışmaz**. Kiracı bazında yönetici beyaz listesi. Yöneticiler kendi web uygulamalarını ekler (v3.40.0). |
...
| **Rota planlayıcı** | Şarj süresi ve kWh tahmini dahil SoC farkındalıklı şarj duraklarıyla etkileşimli rota planlayıcı; kalkış SoC'si (canlı veya manuel), hedef SoC ve şarj hedefi yapılandırılabilir; rota boyunca hava durumu (Open-Meteo), trafik (HERE Maps), hız kameraları (OpenStreetMap); döşeme (tile) proxy'li harita görünümü. **Rota planlayıcı istek listesi (v3.51.4–.8):** toplam seyahat süresi + bölüm bazlı döküm, manuel mola girişi (varış saatine + ICS dışa aktarımına yansır), şarj istasyonları için operatör filtresi, şarj istasyonlarına dair OpenChargeMap topluluk yorumları |
...
Script mimariyi otomatik tespit eder (x86_64, ARM64) ve her şeyi kurar.
