...
- **Vehicle model derived from the VIN, battery capacity taken from the vehicle.** The Fleet API's vehicle list carries no `model_name` — the old Owner API did. On Fleet instances `vehicles.model` therefore stayed empty forever, with two consequences: the WLTP comparison column in the trip list always showed "—", and the energy estimate used a **hard-coded Model Y capacity of 75 kWh** — about 20 kWh off for a Model S, and that error feeds straight into the reported consumption.

  The information was there all along: the VIN encodes the series in position 4, and `trim_badging` carries the capacity as a number ("p74d" = Performance, 74 kWh, dual motor). New `services/vehicleModel.js` with `modelFromVin()`, `resolveModel()` and `usableBatteryKwh()`; the latter prefers the badging (vehicle-exact), then a per-series value, then the previous default. A badging without a plausible number (40–130 kWh) is discarded rather than letting a model year pass as a capacity.

  Vehicle sync now stores the model and **never overwrites an existing value with NULL** (otherwise the next sync would blank what was just filled in). Existing vehicles without a model are backfilled once from the VIN by nightly maintenance.
...
- **Nightly maintenance left a WAL file the size of the database behind, every night.** Per tenant it ran `wal_checkpoint(TRUNCATE)` first and `VACUUM` second — and VACUUM writes the **entire** database into the WAL. That WAL then sat there until the next run: a 58 MB database next to a 58 MB WAL, twice the disk footprint, recreated nightly. On the master DB the checkpoint always came after the VACUUM; on tenant DBs it came before. It now truncates again after the VACUUM.

  Measured on a copy of a real database: checkpoint → WAL 0.0 MB, VACUUM → WAL 58.0 MB, second checkpoint → WAL 0.0 MB.

- **The checkpoint could fail silently.** It ran through `exec()`, whose return value nobody looked at. SQLite reports `{busy, log, checkpointed}` and does nothing while a reader holds a snapshot — a checkpoint that does nothing and says nothing lets the WAL grow until disk space makes it someone's problem. It now runs through `pragma()` with the result evaluated; a `busy` lands in the maintenance audit as `wal_checkpoint_busy`. Each run also records the WAL size (`wal_bytes`), so a backlog becomes visible instead of having to be discovered.

> Release note: the code for this version already shipped with PR #313, which was missing its version bump and changelog entry — both are caught up here. Between that merge and this version `/api/health` therefore reported `3.56.2` while the fix was already running.
...
- **The kWh total now says what it rests on.** When not every trip in the period carries an energy value (GPS-only trips don't), a `*` marks the figure and the tooltip gives the ratio: "sum over 8 of 12 trips — actual consumption is therefore higher". Without it the number reads as total consumption while trips are missing. `/api/trips/stats` returns a new `energy_trips` field for this.
...
- **Route planner: community comments on charging stations (fifth and final item of the route planner wish list).** Opening a charging-station popup now lazy-loads and shows up to 3 recent OpenChargeMap user comments (author, date, text) — users sometimes mention amenity details like restroom cleanliness in the free text there. **Deliberately no own ratings database:** TeslaView is a multi-tenant self-hoster without a central server; a real cross-instance community rating would need a central service that doesn't exist. OpenChargeMap also has no structured amenity field (no dedicated restroom/facility data field) — only free-text `UserComments`. New endpoint `GET /api/routing/chargers/:id/comments` queries only the clicked station on demand (`chargepointid` + `includecomments=true`), deliberately not bulk on the map's station list (performance). **Security:** community free text is uncurated third-party content — every value (name, operator, comment author, comment text) is escaped before being interpolated into the map popup's HTML, verified live with a `<script>` injection test comment (lands as harmless text, no executable element). i18n x7 (4 new keys).
...
- **Consumption display showed "0.0 kWh/100km" instead of "–" for trips without vehicle data.** In `Trips.vue`, `Dashboard.vue`, and `TripDetail.vue`, the consumption calculation only checked `distance_km`, not `energy_used_kwh` — with a `null` energy value, `null / distance_km * 100` evaluates to `0` in JavaScript, producing a misleading consumption figure instead of an honest "–". This mainly affects trips without a Tesla vehicle connection: `owntracks.js` deliberately writes neither `energy_used_kwh` nor `start_soc`/`end_soc` when closing a GPS-only trip (smartphone tracking), since no vehicle data exists for those. Since Tesla disabled the Owner API community workaround in May/June 2026, part of trip tracking now runs through this GPS-only path exclusively — which is why the display bug appeared "always" (user report). The table under `/fahrtwerte` already had this guard correctly (backend `trips.js`); now made consistent across all three frontend spots. Not a data-collection bug — "–" is the honest answer for trips without vehicle telemetry.
...
- **Trip metrics table: missing columns were only unscrolled.** The table under `/fahrtwerte` has 12 columns and is wider than the viewport on most screens. macOS and iOS hide the scrollbar by default, so there was no visible cue that more columns existed to the right — it looked like columns were missing (user report: "can't see all columns"). Measured: 179px of overflow at a 1440px viewport width.
  ...
  The table now shows a right-edge fade hint that only appears while unseen columns remain and disappears automatically once scrolled all the way through. From tablet width up (`sm:`, ≥640px), the date column also stays pinned while scrolling horizontally so the row context is preserved. On phone width it stays unpinned, since the pinned column plus the fade would together have occupied 42.5% of the viewport (measured at 390px) — there the fade hint alone solves discoverability. No column is hidden at any breakpoint.
...
- **Self-hosted web fonts instead of Google — privacy.** Bricolage Grotesque, Manrope and JetBrains Mono (the "editorial" design) were loaded live from `fonts.googleapis.com`/`fonts.gstatic.com`, which would send every visitor's IP to Google. The fonts are now bundled as variable `woff2` (SIL OFL 1.1, including latin/-ext, cyrillic/-ext, greek/-ext subsets for all 7 UI languages) under `frontend/public/fonts/` and served same-origin (`/fonts/fonts.css`). The Google `<link>` and `preconnect` are removed from `index.html` — **the app no longer contacts Google at all**. Consequently the **CSP is strict again** (`style-src 'self' 'unsafe-inline'`, `font-src 'self'`) — the font opening required in v3.36.7 is reverted. License text + attribution: `frontend/public/fonts/LICENSE.md`.
...
- **Guarded a `null.toFixed` render crash.** Several spots called `.toFixed()` directly on potentially-empty fields (preconditioning suggestion without a temperature in `Battery.vue`, hover/summary km in `TripsHeatmap.vue`, avg/peak kW tooltip in `ChargingHeatmap.vue`). When the value was missing, the render threw `Cannot read properties of null (reading 'toFixed')`. All affected calls are now guarded with `?? 0`.
...
- **Logout on every reload — the actual cause: a router-guard vs session-restore race (#14).** The server side was fine all along (`/auth/refresh` **and** `/auth/me` demonstrably returned `200` on reload, cookies were correct) — yet you still landed on the login page. Reason: the router's initial navigation ran **in parallel** with session restoration. The `beforeEach` guard read `isAuthenticated` at the moment `refresh` was still in flight and `this.user` wasn't set yet → `false` → redirect to `/login`. When `refresh`+`me` came back `200` moments later, the app was already on `/login` and never navigated back. Fix: a new shared promise `ensureSessionRestored()` in `store/auth.js` — boot (`main.js`) **and** the router guard await the **same** one-time restore before reading `isAuthenticated`. Exactly **one** `/auth/refresh` fires (no refresh-token rotation race). This confirms the earlier attempts (v3.36.4 `no-store`, v3.36.6 service worker) were misdiagnoses — both stay in as reasonable hardening.
...
- **Logout on every reload — the real cause was in the service worker, not the server.** The `no-store` fix from v3.36.4 didn't solve it because the problem was client-side: the SW `fetch` handler also tried to cache foreign requests (e.g. `chrome-extension://` from browser extensions) — `cache.put` then throws "unsupported scheme". On top of that, the navigation fallback could return `undefined` ("Failed to convert value to 'Response'"). Both made the HTML navigation fail on reload → the app never booted → the session was never restored → login screen. Fix in `sw.js`: only same-origin GETs are handled, every `cache.put` is guarded against errors, and the navigation fallback **always** returns a valid response. Cache version `tcv-v4` → `tcv-v5`.
- **Removed the temporary auth diagnostic (`GET /api/auth/_diag`) for good.** It was re-enabled (secret-gated) in v3.36.5 to narrow down #14; cause now confirmed (SW-side), security finding resolved.

### Changed

- **CSP now allows Google Fonts** (`styleSrc`: `fonts.googleapis.com`, `fontSrc`: `fonts.gstatic.com`) — the console CSP blockers when loading the app font are gone.
