...
- 🗓️ **Aperçus hebdomadaires** — carte « Votre semaine » avec des indications claires sur le kilométrage, la consommation vs moyenne sur 90 jours (avec justification liée au froid), le coût de recharge et les anomalies ouvertes (v3.30, statistiques pures ; amélioration optionnelle par LLM local avec Ollama)
- 🛞 **Tendance de pression des pneus** — détecte une perte de pression lente par pneu (compensée en température) et alerte tôt (v3.33)
- 🍃 **Score de conduite** — évalue l'efficience de votre conduite par rapport à votre propre moyenne, avec des conseils d'économie fondés sur les données (v3.34, statistiques pures)
- 🔌 **Courbe de charge en direct** — la session de recharge en cours en temps réel avec puissance, état de charge et courbe attendue (v3.35)
...
Au-dessus de la liste des trajets se trouve une **barre de période** : choix rapides (7 jours, 30 jours, ce mois-ci, mois dernier, cette année) ou bornes libres **du/au**. À droite s'affiche le **total de la période** — kilomètres, énergie consommée et nombre de trajets. Si les trajets de la période ne portent pas tous une valeur d'énergie — les trajets uniquement GPS, sans données du véhicule, n'en ont pas —, le chiffre en kWh reçoit un `*` et l'infobulle indique sur combien de trajets repose la somme. La consommation réelle est alors plus élevée. Il en va de même pour l'énergie affichée dans la barre de sélection. Ce total est calculé côté serveur sur *tous* les trajets de la période, pas seulement sur les 50 chargés : « charger plus » ne le modifie pas. Les cartes statistiques suivent la même période, ainsi que les filtres de type et de conducteur.
...
- Serveur Linux (x86_64 ou ARM64) avec au moins 1 Go de RAM
...
- **L'enregistrement partenaire unique chez Tesla est pris en charge automatiquement par l'assistant** (depuis la v3.23.5) — vous saisissez seulement Client ID + Secret, plus besoin d'appel `curl` manuel.
...
| **Identifiants Tesla** | Saisir Client ID + Secret — TeslaView **enregistre alors l'app chez Tesla automatiquement** (1 clic, sans `curl`) ; le domaine détecté est confirmé une fois |
...
## 📍 À proximité (à partir de v3.13.0) {#nearby}

`/nearby` affiche les POIs (points d'intérêt) autour de votre voiture, de votre session de charge active ou de la fin de votre dernier trajet. Pratique lors des arrêts charge rapide.

**Catégories** : café, restaurant, restauration rapide, boulangerie, supermarché, toilettes, eau potable, aire de jeux, parc, pique-nique, point de vue, DAB, pharmacie, **geocaches**.

**Source** : [OpenStreetMap Overpass API](https://overpass-api.de) — gratuite, sans compte, sans clé API. Les appels passent côté serveur et les résultats sont mis en cache 24 h dans `poi_cache` (arrondis à 4 décimales → ~11 m).

**Rayon** : 500 m / 1,5 km / 3 km. Cliquez sur un POI pour l'ouvrir dans OpenStreetMap.

**Filtre** : chaque catégorie est un toggle — par ex. seulement les geocaches pour une chasse au trésor pendant la charge.
...
**Les pages se chargent à moitié – seul un rechargement aide**
Un message « le serveur limite les requêtes » apparaît généralement brièvement en bas à droite. Depuis la v3.52.4, l'application relance automatiquement les requêtes concernées. Si le problème persiste – surtout juste après un zoom sur une carte –, la limite de débit du proxy inverse placé devant est trop stricte. Pour les administrateurs : `docs/02-deployment.fr.md`, section « Exploitation derrière votre propre proxy inverse ».
