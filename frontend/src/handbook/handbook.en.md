...
**Heatmap** (navigation → *Heatmap*) shows on a map *where* you drive — with four individually toggleable layers: **trips** (density of start/end points), **charging sessions**, your defined **charging spots** and **routes** (the GPS routes of the trips as lines, up to the 300 most recent trips in the range). Time range (30/90/365 days/all) is selectable; the map auto-fits to the visible points. The **colour of each layer** is freely adjustable via the colour dot next to the option (stored in the browser, "↺ Default colours" resets).
...
Above the trip list sits a **period bar**: quick presets (7 days, 30 days, this month, last month, this year) or free **from/to** bounds. On the right you get the **period total** — distance, energy used and trip count. When not every trip in the period carries an energy value — GPS-only trips without vehicle data don't — the kWh figure gets a `*`, and the tooltip says how many trips the sum covers. Actual consumption is higher in that case. The same applies to the energy figure in the selection bar. That total is calculated on the server across *all* trips in the period, not just the 50 loaded ones, so "load more" doesn't change it. The stat tiles below follow the same period plus the type and driver filters.
...
- Linux server (x86_64 or ARM64) with at least 1 GB RAM
...
- **The one-time partner registration with Tesla is handled by the wizard automatically** (since v3.23.5) — you only enter Client ID + Secret, no manual `curl` call needed.
...
| **Tesla credentials** | Enter Client ID + Secret — TeslaView then **registers the app with Tesla automatically** (1 click, no `curl`); the detected domain is confirmed once |
...
### Setup on Android (instead of iOS)

Android has no native 1:1 replacement for iOS Shortcuts. Three ways:

**Recommended: MacroDroid** (free version is enough, ~10M downloads, safe)
1. Install from the Play Store
2. "+ New macro" → trigger "Bluetooth" → "Connected to device" → pick the Tesla
3. Action "HTTP request" → method GET → paste the connect URL from TeslaView
4. Save as "Tesla connected"
5. Second macro the same way for "Bluetooth disconnected" with the disconnect URL

**Alternatives:**
- **Automate (Llamalab)** — free up to 30 blocks, visual flow (cleaner but learning curve)
- **Tasker** — €3.49 one-off, gold standard for Android automation

⚠ **Verification note:** This guide has not been verified live on an Android device (developer side is iOS only). If something is off, please open a GitHub issue — we'll iterate.
...
## 📍 Nearby (from v3.13.0) {#nearby}

`/nearby` shows POIs (points of interest) around your car, your active charging session or your last trip end. Handy during fast-charge stops ("Where's the next toilet?", "Café around the corner?").
...
**Data source**: [OpenStreetMap Overpass API](https://overpass-api.de) — free, no account, no API key. Calls happen server-side and results are cached in `poi_cache` for 24 hours (rounded to 4 decimals → ~11 m). Minimal load on OSM.

**Radius**: 500 m / 1.5 km / 3 km. Clicking a POI opens OpenStreetMap in the browser.

**Filter**: every category as a toggle — e.g. show only geocaches for a treasure hunt during charging.
