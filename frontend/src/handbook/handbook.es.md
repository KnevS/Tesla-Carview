...
- 🛞 **Tendencia de presión de neumáticos** — detecta pérdidas lentas de presión por neumático (compensadas por temperatura) y avisa a tiempo (v3.33)
- 🍃 **Puntuación de conducción** — evalúa la eficiencia de tu conducción frente a tu propia media, con consejos de ahorro basados en datos (v3.34, pura estadística)
- 🔌 **Curva de carga en vivo** — la sesión de carga en curso en tiempo real con potencia, estado de carga y curva esperada (v3.35)
...
Sobre la lista de viajes hay una **barra de periodo**: selección rápida (7 días, 30 días, este mes, mes pasado, este año) o límites libres **desde/hasta**. A la derecha aparece el **total del periodo** — kilómetros, energía consumida y número de viajes. Si no todos los viajes del periodo tienen un valor de energía —los viajes solo con GPS, sin datos del vehículo, no lo tienen—, la cifra en kWh lleva un `*` y el tooltip indica de cuántos viajes procede la suma. En ese caso el consumo real es mayor. Lo mismo vale para la energía en la barra de selección. Ese total se calcula en el servidor sobre *todos* los viajes del periodo, no solo sobre los 50 cargados: «cargar más» no lo cambia. Las tarjetas de estadísticas siguen el mismo periodo y además los filtros de tipo y conductor.
...
## 📅 Planificador de carga {#charge-planner}

El **planificador de carga** (nav "Planificación → Planificador de carga", `/ladeplan`) responde a la pregunta «¿cuándo debo cargar?» para una tarifa eléctrica dinámica. Elige de la curva de precios las **horas más baratas antes de la salida** —no necesariamente consecutivas— en lugar de simplemente la siguiente franja consecutiva.

**Requisito** — En Ajustes → Tarifa debe haber un proveedor dinámico (aWattar o Tibber) conectado. Sin tarifa, el planificador muestra un aviso con enlace a la configuración.

**Entradas** — Carga actual, carga objetivo, capacidad de la batería (kWh), potencia de carga (kW) y hora de salida. Los valores se recuerdan localmente en el navegador.

**Resultado** — Cuatro cifras: energía a cargar (incl. la carga alcanzada), tiempo de carga puro, coste óptimo (solo horas más baratas) y el ahorro frente a cargar de inmediato (absoluto y en %). Debajo, un gráfico de barras muestra la ventana de planificación hasta la salida; las horas de carga elegidas por el planificador se resaltan en verde.

**Tiempo insuficiente** — Si la ventana antes de la salida es demasiado corta para la carga objetivo, el planificador lo indica y muestra la carga máxima alcanzable.
...
**Las páginas se cargan a medias: solo ayuda recargar**
Normalmente aparece brevemente abajo a la derecha el aviso «el servidor está limitando las solicitudes». Desde la v3.52.4 la aplicación reintenta automáticamente las peticiones afectadas. Si persiste —sobre todo justo después de hacer zoom en un mapa—, el límite de tasa del proxy inverso es demasiado estricto. Para administradores: `docs/02-deployment.es.md`, sección «Funcionamiento detrás de un proxy inverso propio».
