  ...
  <div class="max-w-2xl space-y-6">
    ...
    <div class="card space-y-3">
      ...
      <div v-else class="space-y-2">
        ...
        <RouterLink to="/mfa/setup" class="btn-primary inline-block text-sm">MFA aktivieren</RouterLink>
      </div>
    </div>

    <!-- Passwort aendern -->
    <div class="card space-y-3">
      <h2 class="font-semibold">🔑 Passwort aendern</h2>
      <div class="space-y-2">
        ...
        <button @click="changePassword" class="btn-primary text-sm">Passwort aendern</button>
      </div>
    </div>

    <!-- Audit-Log -->
    <div class="card space-y-3">
      <h2 class="font-semibold">📝 Letzte Aktivitaeten</h2>
      <div class="space-y-1">
        <div v-for="e in auditLog" :key="e.created_at"
          class="flex justify-between text-sm py-1 border-b border-gray-700 last:border-0">
          <span class="font-mono text-gray-300">{{ e.action }}</span>
          <span class="text-gray-500">{{ fmtDate(e.created_at) }}</span>
        </div>
    ...
    <!-- Logout -->
    <div class="card">
      <button @click="logout" class="text-red-400 hover:text-red-300 text-sm transition">Abmelden</button>
    </div>
  </div>
</template>
