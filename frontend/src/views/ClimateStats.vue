<template>
  <div class="space-y-4">
    ...
    <template v-for="sid in layoutOrder" :key="sid">
      ...
      <!-- Gesamt-Stats + Extremwerte -->
      <SortableSection v-if="sid === 'stats' && totals"
        page-id="climate" section-id="stats"
        :title="$t('climate.title')" icon="📊"
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="bg-gray-800 rounded-xl p-3 space-y-1"
      ...
      <!-- Tages-Verlauf -->
      <SortableSection v-if="sid === 'chart'"
        page-id="climate" section-id="chart"
        :title="$t('climate.dailyChart')" icon="📅"
        :collapsed="isCollapsed('chart')"
        @toggle="toggle('chart')"
        @move="(f, t, p) => moveSection(f, t, p)">
        <template #badge><InfoTip :text="$t('climate.dailyChartTip')" /></template>
...
const CS_SECTIONS = ['stats', 'chart'];
const { orderedSections: layoutOrder, isCollapsed, toggle, moveSection } = usePageLayout('climate', CS_SECTIONS);
const dayData  = ref([]);
