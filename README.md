...
Läuft auf: **Linux-Server** (x86_64), **Raspberry Pi 4/5** (ARM64), **Windows** (Docker Desktop + WSL2, ohne Fahrzeugbefehle), lokale Entwicklung.
...
| **Ladepreis-Ranking** (v3.50.0) | „Wo lädst du am günstigsten?" — sortiert Ladeorte nach €/kWh statt nach Gesamtausgaben und rechnet den Aufpreis gegen den eigenen Heimstrom. Ladungen ohne hinterlegten Tarif zählen bewusst nicht als kostenlos, die Preisbasis wird ausgewiesen |
| **Energiebilanz** (v3.49.0) | „Was verbrauchst du wirklich?" — Teslas Anzeige zeigt nur den Fahrtverbrauch. Die Bilanz stellt Fahrt, Akku-zu-Rad (inkl. Standby) und Netz-zu-Rad (inkl. Ladeverluste) nebeneinander; typisch 15–25 % Unterschied. Ladestandskorrektur mit gemessener Akkukapazität; geht die Bilanz nicht auf, erscheint bewusst keine Zahl statt einer irreführenden |
...
| **Dienstwagen-Versteuerung** (v3.45.0) | 1-%-Regel vs. Fahrtenbuchmethode für den geldwerten Vorteil — mit datumsabhängigem E-Fahrzeug-Satz (0,25 %/0,5 %, BLP-Grenze nach Anschaffungsdatum). Nur reine E-Fahrzeuge (BEV), Orientierungsrechnung, nur Dienstwagen, deutschsprachig |
...
| **Routenplaner** | Interaktiver Routenplaner mit SoC-aware Ladeplanung: automatische Ladestopps inkl. Ladezeit- und kWh-Schätzung; Abfahrts-SoC (live oder manuell), Ziel-SoC und Ladeziel konfigurierbar; Wetter (Open-Meteo), Verkehr (HERE Maps), Blitzer (OpenStreetMap) entlang der Route; Kartendarstellung mit Tile-Proxy. **Persönliche Reichweite (v3.29): Ankunfts-SoC aus deinem temperatur-abhängigen Verbrauch statt WLTP, mit Vertrauensband und „Könnte knapp werden"-Warnung — reine Statistik.** **Routenplaner-Wunschzettel (v3.51.4–.8):** Gesamtreisezeit + Teilstrecken-Aufschlüsselung, manuelle Pausen-Eingabe (fließt in Ankunftszeit + ICS-Export), Anbieter-Filter für Ladestationen, OpenChargeMap-Community-Kommentare zu Ladestationen |
