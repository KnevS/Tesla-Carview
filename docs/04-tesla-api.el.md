...
## Εγγραφή συνεργάτη — αυτόματα μέσω του οδηγού (συνιστάται)

Από την **έκδοση v3.23.5** **δεν χρειάζεται πλέον να καταχωρείτε την
εφαρμογή στην Tesla με το χέρι μέσω `curl`**. Ο οδηγός εγκατάστασης (Κέντρο
διαχείρισης → 🛠️) το κάνει μόνος του:

1. Στο βήμα **«Διαπιστευτήρια Tesla Fleet API»**, εισαγάγετε το Client ID +
   Client Secret (από την [πύλη προγραμματιστών της Tesla](https://developer.tesla.com)).
2. Το TeslaView εμφανίζει από κάτω τον **εντοπισμένο τομέα** της
   εγκατάστασής σας, για μια εφάπαξ επιβεβαίωση.
3. Κάντε κλικ στο **«🔑 Εγγραφή στην Tesla τώρα»** — ή απλώς στο «Επόμενο»,
   και ο οδηγός εγγράφει αυτόματα.

Στο παρασκήνιο, ο διακομιστής λαμβάνει ένα διακριτικό `client_credentials`
και καλεί το `POST /api/1/partner_accounts` με τον τομέα σας — ακριβώς ό,τι
θα έκανε μια χειροκίνητη κλήση `curl`. Η επιτυχία απομνημονεύεται (`✓
Εγγεγραμμένο για <τομέας>`)· μετά από αλλαγή τομέα, ο οδηγός προτείνει εκ
νέου εγγραφή.

> Endpoint: `POST /api/fleet/partner/register`. Προσβάσιμο επίσης από
> Ρυθμίσεις διαχειριστή → ⚡ Σύνδεση Tesla.

### Υγιεινή ασφάλειας

Ο αυτοματισμός έχει σχεδιαστεί σκόπιμα ώστε να μη διαρρέουν μυστικά και να
μην παρεισφρέει καμία λανθασμένη διαμόρφωση:

- **Το Client Secret δεν φεύγει ποτέ από τον διακομιστή.** Αποθηκεύεται
  κρυπτογραφημένο στο `tenant_settings` (κλειδί: `data/.encryption-key`),
  διαβάζεται στην πλευρά του διακομιστή και αποστέλλεται μόνο στο σημείο
  διακριτικών της Tesla — ποτέ δεν επιστρέφεται στον περιηγητή.
- **Ο τομέας δεν επιλέγεται ελεύθερα.** Εγγράφεται πάντα ο τομέας
  λειτουργίας (`FRONTEND_URL`)· μια τιμή που στέλνεται από τον περιηγητή
  είναι μόνο εφεδρική, εάν λείπει το `FRONTEND_URL`. Αυτό αποτρέπει την κατά
  λάθος εγγραφή λανθασμένου τομέα — η Tesla τον επαληθεύει ούτως ή άλλως
  ανακτώντας το δημόσιο κλειδί στο
  `https://<τομέας>/.well-known/appspecific/com.tesla.3p.public-key.pem`.
- **Μόνο διαχειριστές** μπορούν να ενεργοποιήσουν την εγγραφή.
- **Ταυτοδύναμο (idempotent).** Η εκ νέου κλήση είναι ακίνδυνη — η Tesla
  απλώς ενημερώνει την υπάρχουσα καταχώριση.

> Αν προτιμάτε το `.env`, μπορείτε ακόμα να ορίσετε εκεί τα διαπιστευτήρια
> (δείτε παρακάτω) — ο οδηγός τα λαμβάνει αυτόματα.
